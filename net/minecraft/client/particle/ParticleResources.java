 ...
 */
    ...
    public CompletableFuture<Void> reload(
        PreparableReloadListener.SharedState p_446196_, Executor p_445402_, PreparableReloadListener.PreparationBarrier p_446595_, Executor p_445639_
    ) {
        ResourceManager resourcemanager = p_446196_.resourceManager();
        ...
        CompletableFuture<List<ParticleDefinition>> completablefuture = CompletableFuture.<Map<Identifier, Resource>>supplyAsync(
                () -> PARTICLE_LISTER.listMatchingResources(resourcemanager), p_445402_
            )
            .thenCompose(
                p_465571_ -> {
                    List<CompletableFuture<ParticleDefinition>> list = new ArrayList<>(p_465571_.size());
                    p_465571_.forEach(
                        (p_465580_, p_465581_) -> {
                            Identifier identifier = PARTICLE_LISTER.fileToId(p_465580_);
                            list.add(
                                CompletableFuture.supplyAsync(
                                    () -> new ParticleDefinition(identifier, this.loadParticleDescription(identifier, p_465581_)), p_445402_
                                )
                            );
                        }
                    );
        ...
        CompletableFuture<SpriteLoader.Preparations> completablefuture1 = p_446196_.get(AtlasManager.PENDING_STITCH).get(AtlasIds.PARTICLES);
        return CompletableFuture.allOf(completablefuture, completablefuture1).thenCompose(p_446595_::wait).thenAcceptAsync(p_446487_ -> {
            ...
            completablefuture.join().forEach(p_465575_ -> {
                Optional<List<Identifier>> optional = p_465575_.sprites();
                if (!optional.isEmpty()) {
                    ...
                    this.spriteSets.get(p_465575_.id()).rebind(list);
                }
            });
            if (!set.isEmpty()) {
    ...
    private Optional<List<Identifier>> loadParticleDescription(Identifier location, Resource resource) {
    ...
    static class MutableSpriteSet implements SpriteSet {
        ...
        @Override
        public TextureAtlasSprite get(int p_445708_, int p_446170_) {
            return this.sprites.get(p_445708_ * (this.sprites.size() - 1) / p_446170_);
        }

        @Override
        public TextureAtlasSprite get(RandomSource p_445928_) {
            return this.sprites.get(p_445928_.nextInt(this.sprites.size()));
        }
