...
class StickyAsh(props: Item.Properties) : Item(props), ProjectileItem {
    override fun use(level: Level, player: Player, hand: InteractionHand): InteractionResult {
        ...
        if (level is ServerLevel) {
            val projectile = ThrownStickyAsh(level, player, itemStack.copyWithCount(1))
    ...
    override fun asProjectile(level: Level, position: Position, itemStack: ItemStack, direction: Direction): net.minecraft.world.entity.projectile.Projectile =
        ThrownStickyAsh(level, position.x(), position.y(), position.z(), itemStack)

    companion object {
        @JvmField
        val PROJECTILE_SHOOT_POWER: Float = 1.5f

        @JvmField
        val PROJECTILE_INACCURACY: Float = 3.0f
    }
}
