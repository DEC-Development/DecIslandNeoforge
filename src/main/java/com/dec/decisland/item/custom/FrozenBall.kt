...
class FrozenBall(props: Item.Properties) : Item(props), ProjectileItem {
    override fun use(level: Level, player: Player, hand: InteractionHand): InteractionResult {
        ...
        if (level is ServerLevel) {
            val projectile = FrozenBallProjectile(level, player, itemStack.copyWithCount(1))
    ...
    override fun asProjectile(level: Level, position: Position, itemStack: ItemStack, direction: Direction): Projectile =
        FrozenBallProjectile(level, position.x(), position.y(), position.z(), itemStack)

    companion object {
        const val PROJECTILE_SHOOT_POWER: Float = 1.9f
        const val PROJECTILE_INACCURACY: Float = 3.0f
    }
}
