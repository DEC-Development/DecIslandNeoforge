...
class ThrowableBomb(props: Item.Properties) : Item(props), ProjectileItem {
    override fun use(level: Level, player: Player, hand: InteractionHand): InteractionResult {
        ...
        if (level is ServerLevel) {
            val projectile = ThrowableBombProjectile(level, player, itemStack.copyWithCount(1))
    ...
    override fun asProjectile(level: Level, position: Position, itemStack: ItemStack, direction: Direction): Projectile =
        ThrowableBombProjectile(level, position.x(), position.y(), position.z(), itemStack)

    companion object {
        const val PROJECTILE_SHOOT_POWER: Float = 0.7f
