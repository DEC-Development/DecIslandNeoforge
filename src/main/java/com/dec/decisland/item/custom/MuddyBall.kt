...
class MuddyBall(props: Item.Properties) : Item(props), ProjectileItem {
    override fun use(level: Level, player: Player, hand: InteractionHand): InteractionResult {
        ...
        if (level is ServerLevel) {
            val projectile = MuddyBallProjectile(level, player, itemStack.copyWithCount(1))
    ...
    override fun asProjectile(level: Level, position: Position, itemStack: ItemStack, direction: Direction): Projectile =
        MuddyBallProjectile(level, position.x(), position.y(), position.z(), itemStack)

    companion object {
        const val PROJECTILE_SHOOT_POWER: Float = 1.6f
