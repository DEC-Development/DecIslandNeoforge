...
class FireflyBottle(props: Item.Properties) : Item(props), ProjectileItem {
    override fun use(level: Level, player: Player, hand: InteractionHand): InteractionResult {
        ...
        if (level is ServerLevel) {
            val projectile = FireflyBottleProjectile(level, player, itemStack.copyWithCount(1))
            projectile.setPos(player.x, player.eyeY - 0.1, player.z)
            projectile.shootFromRotation(player, player.xRot, player.yRot, -20.0f, PROJECTILE_SHOOT_POWER, PROJECTILE_INACCURACY)
    ...
    override fun asProjectile(level: Level, position: Position, itemStack: ItemStack, direction: Direction): Projectile =
        FireflyBottleProjectile(level, position.x(), position.y(), position.z(), itemStack)

    companion object {
        const val PROJECTILE_SHOOT_POWER: Float = 0.5f
        const val PROJECTILE_INACCURACY: Float = 1.0f
    }
}
