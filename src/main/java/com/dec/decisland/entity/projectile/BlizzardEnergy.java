...
public class BlizzardEnergy extends Projectile {
    public BlizzardEnergy(EntityType<BlizzardEnergy> entityType, Level level) {
        super(entityType, level);
    }
    public BlizzardEnergy(Level level, LivingEntity owner, ItemStack spawnedFrom) {
        this(ModEntities.BLIZZARD_ENERGY.get(), level);
        this.setOwner(owner);
        this.setPos(
    ...
    public void tick() {
        ...
        if (this.level().getBlockStates(this.getBoundingBox()).noneMatch(BlockBehaviour.BlockStateBase::isAir)) {
            ...
            this.setDeltaMovement(vec3.scale(1.2F));
            this.applyGravity();
    ...
    protected void onHitEntity(EntityHitResult result) {
        ...
        if (this.getOwner() instanceof LivingEntity livingentity) {
            ...
            if (this.level() instanceof ServerLevel serverlevel && entity.hurtServer(serverlevel, damagesource, 9)) {
