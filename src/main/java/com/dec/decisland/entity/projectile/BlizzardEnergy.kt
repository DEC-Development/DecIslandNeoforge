...
class BlizzardEnergy(entityType: EntityType<BlizzardEnergy>, level: Level) : Projectile(entityType, level) {
    ...
    override fun tick() {
        ...
        if (level().getBlockStates(boundingBox).noneMatch(BlockBehaviour.BlockStateBase::isAir)) {
            ...
            setDeltaMovement(vec3.scale(1.2))
            applyGravity()
            setPos(d0, d1, d2)
        }
    ...
    override fun onHitEntity(result: EntityHitResult) {
        ...
        if (owner is LivingEntity) {
            ...
            if (currentLevel is ServerLevel && entity.hurtServer(currentLevel, damageSource, 9.0f)) {
